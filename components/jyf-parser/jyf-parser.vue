...
<script>
	...
	var rpx = uni.getWindowInfo().screenWidth / 750,
	...
	export default {
		...
		provide() {
			return {
				registerTrees: (instance) => {
					this.treesRegistry.push(instance)
				},
				unregisterTrees: (instance) => {
					const i = this.treesRegistry.indexOf(instance)
					if (i > -1) this.treesRegistry.splice(i, 1)
				}
			}
		},
		// #ifndef H5 || APP-PLUS-NVUE
		...
		methods: {
			...
			setContent(html, append) {
				...
				this.$nextTick(() => {
					...
					setTimeout(() => {
						// #endif
						this.treesRegistry.forEach((c) => {
							if (c.$options.name != 'trees') return
			...
			_load(e) {
				if (this._wait.length)
					this.imgs[e.target.id] = this._wait.shift();
			},
			// #endif
			_tap(e) {
				// #ifndef MP-BAIDU || MP-ALIPAY || APP-PLUS
